...
# which is here:
getwd()
sub_UKBB<-read.csv("/Users/roxydong/MyRproject/data/UKB_phenotype/data_cleaned.csv")
```

**Exploratory data analysis (EDA) of the data**
```{r}
dim(sub_UKBB) 
# This data covering 144756 participants and 11 variables of them (IID, FID, etc)
```
...
The vairables were re-classified in to "yes", "no" and "NA" and recodeded as the dummy variables.
...
Patients have more than one records of 4803 would have more than one age records.

**Q7:** For those with more than one age records, which age do you think should be used in the genetic association analysis?

Age diagnosed with tinnitus, if missing then baseline age.
...
Yes. Negative. Significant, p-value: < 2.2e-16
```{r, echo=FALSE}
...
They were negatively associated. The association is significant.

```{r Q10}
auto_data$mpg_cat <- ifelse(auto_data$mpg>23, 1, 0)
fit_2 <- glm(mpg_cat ~ weight, family = "binomial", data=auto_data)
summary(fit_2)
```
...
(1) One unit increase in weight will result -0.0076 decrease in mpg.

(2) One unit increase in weight, the log odds of mag ( >23 versus <=23) decreases by 0.0041.
